(function(angular, matchupData, sortData, radarChartSettings, lineChartSettings) {
    ...
    appChampion.controller('championData', ['$scope', function($scope) {
		...
		var roleTitle = '';
		if($scope.champion.roleTitle == 'Middle') {
			roleTitle = '中单';
		} else if($scope.champion.roleTitle == 'Jungle') {
			roleTitle = '打野';
		} else if($scope.champion.roleTitle == 'Top') {
			roleTitle = '上单';
		} else if($scope.champion.roleTitle == 'Support') {
			roleTitle = '辅助';
		} else {
			roleTitle = $scope.champion.roleTitle;
		}
        // First graph
        ...
        $scope.experienceRate = {
            data: {
                ...
                datasets: [{
                    ...
                    label: $scope.champion.name + " 总体胜率",
                    fillColor: "rgba(220,220,220,0.3)",
                    strokeColor: "rgba(220,220,220,1)",
                    pointColor: "rgba(220,220,220,1)",
                    pointStrokeColor: "#fff",
